...
(function () {
  ...
  /* Мобильный вьюпорт: у страницы НЕТ meta viewport, поэтому телефоны рендерят её
     ...
     ⚠️ ТОЛЬКО при мобильном UA (2026-07-17). Раньше мета ставилась безусловно, и
     режим «Версия для ПК» в мобильном Chrome разваливался: сервер по десктопному
     UA отдаёт ДЕСКТОПНЫЙ HTML, а наша мета отключала браузерный фолбэк 980px — и
     десктопная вёрстка (рельса, отступы, min-width'ы) рендерилась в честные
     ~412px кашей. Без меты браузер сам уложит 980px в экран с уменьшением —
     «десктоп в миниатюре», ровно то, зачем эту галку ставят.
     Проверять DOM (.mobile_categories_list) здесь нельзя — мы выполняемся до
     парсинга body. UA — сигнал ровно той же силы: сервер выбирает HTML по нему.
     Регексп повторяет серверную развилку: Android-планшеты БЕЗ токена Mobile
     тоже получают мобильный HTML (проверено ранее) — поэтому «Android», а не
     «Mobile». iPad с «Версией для ПК» шлёт Mac-овский UA → мета не ставится →
     корректный десктоп, как и было. */
  (function ensureViewport() {
    if (!/Android|iPhone|iPod|Mobile/i.test(navigator.userAgent)) return;
  ...
  function localizeChartMonths(c) {
    ...
    for (var i = 0; i < axes.length; i++) {
      ...
      /* ⚠️ redraw:true ОБЯЗАТЕЛЕН. Раньше стояло false в расчёте на то, что
         перерисует идущий следом setSize. Но после появления гарда по размеру
         (см. reflowCharts) setSize почти всегда пропускается — размер-то не
         менялся, — и перерисовки не происходило вовсе. Ось оставалась
         недорисованной: подписи месяцев не отображались, а поиск точки под
         курсором (kdTree) не работал, из-за чего пропадали всплывающие
  ...
  function tagRailIcons() {
    var MAP = [
      ...
      [/филиал/i, "branches"],
      /* «Сотрудники» использует ТУ ЖЕ иконку, что и «Филиалы» (реш. клиента) —
         тот же ключ branches, отдельной иконки не заводим. */
      [/сотрудник/i, "branches"],
      [/обратн.*связ/i, "feedback"],
      [/раздел/i, "sections"],
      [/вопрос/i, "questions"],
      [/[сc]водн/i, "summary"],
      [/pdf/i, "pdf"],
      [/артефакт/i, "artifacts"],
      /* «Статус анкет» клиент переименовал в «Апелляции» (2026-08), «Впечатления»
         — в «NPS». Иконки те же (status/impressions), матчим и старые, и новые
         названия, чтобы работало независимо от переименования. */
      [/статус|апелляц/i, "status"],
      [/впечатлен|nps/i, "impressions"],
      [/карта/i, "map"],
  ...
  /* ⚠️ Excel ПОКАЗЫВАЕТ ширину на 5/7 единицы МЕНЬШЕ записанной: в файле
     хранится значение вместе с 5px внутреннего отступа ячейки, а в интерфейсе
     отступ не показывается. Замерено клиентом в самом Excel на двух точках:
     записали 12 → показал 11.29; записали 42.14 → показал 41.43 (разница ровно
     0.714 = 5/7 в обоих случаях). Поэтому прибавляем поправку — иначе заданные
     минимум/максимум окажутся в Экселе на 0.71 уже, чем договаривались. */
  var EXCEL_WIDTH_PADDING = 5 / 7;
  ...
  function buildColoredWorkbook(table) {
    ...
    for (var i = 0; i !== colCount; i++) {
      ...
      ws.getColumn(i + 1).width =
        Math.max(EXPORT_MIN_COL_WIDTH, pxToExcelWidth(widthPx)) +
        EXCEL_WIDTH_PADDING;
    }
  ...
  /* Мобильный режим определяем по факту МОБИЛЬНОГО HTML (сервер отдаёт его по
     ...
     мобильный CSS.
     lk-wide (планшет: короткая сторона экрана больше 550px, не зависит от
     ориентации) с 2026-07-17 — только аварийный откат: в обычной работе планшет
     идёт вариантом A вместе с телефоном, а lk-wide включается лишь при
     kill-switch. Порог нужен, чтобы отличить планшет от телефона в этом откате. */
  ...
  /* Планшет: мобильный HTML, но экран просторный. Считаем по КОРОТКОЙ стороне —
     ...
     ⚠️ С 2026-07-17 на РЕЖИМ больше не влияет: планшет получает тот же вариант A,
     что и телефон (решение клиента — «повёрнутая мобилка под планшет устраивает»).
     Функция осталась ради kill-switch: с lkAppOff="1" планшет откатывается на
     старую планшетную раскладку (lk-wide), и её надо чем-то отличать. */
  ...
  /* Вариант A = ЛЮБОЙ мобильный HTML (телефон и планшет), решение клиента
     2026-07-17: «на планшет закинем мобильную версию, повёрнутая мобилка под
     планшет устраивает». Раньше здесь был isPhone(), а планшет шёл своей веткой
     (lk-wide) — держать две раскладки дорого, и отладка удваивалась.
     Заодно отпал флаг lkAppForce: он существовал только чтобы обойти проверку
     размера там, где screen врёт (Responsive в DevTools, split-screen). Раз
     размер больше ни на что не влияет — обходить нечего.
     ⚠️ Гейт по мобильному HTML обязателен: из рельсы (.mobile_categories_list)
     строится вся навигация варианта A, на десктопном HTML её просто нет. */
  function isAppMode() {
  ...
  function syncModeClasses() {
    ...
    /* «Версия для ПК» на телефоне\планшете: HTML десктопный (галка шлёт
       десктопный UA), но device-media платформы смотрит на ФИЗИЧЕСКИЙ экран и
       продолжает утюжить страницу мобильной инфляцией (шрифты 3-4rem, кнопки
       2rem-паддингами, бургер 60×60). Метку вешаем, когда HTML десктопный, а
       экран — карманный (800px покрывает и планшеты: iPad portrait = 768).
       CSS по html.lk-pc возвращает раздутым правилам десктопные значения. */
    root.classList.toggle(
      "lk-pc",
      !m && window.matchMedia("(max-device-width: 800px)").matches
    );
    /* ⚠️ lk-wide и lk-app ВЗАИМОИСКЛЮЧАЮЩИЕ: первый включает планшетную
       ...
       вёрстка поехала).
       Теперь вариант A забирает и планшет, поэтому `!app` гасит lk-wide всегда —
       кроме одного случая: kill-switch (lkAppOff="1"). Тогда app=false, и
       планшет откатывается на прежнюю планшетную раскладку. Ради этого ветка и
       живёт: весь html.lk-wide CSS — аварийный откат, а не мёртвый код. */
  ...
  function syncFilt() {
    /* Фильтры уезжают в шторку везде, где включён вариант A (телефон И планшет).
       Гейт именно isAppMode(), а не размер: раньше стоял isPhone(), и после
       передачи планшета варианту A получилась бы каша — таб-бар с кнопкой
       «Фильтры» есть, а сами фильтры остались бы инлайн в потоке.
       При kill-switch (app=false) планшет и десктоп держат фильтры инлайн.
